pipeline {
  ...
  stages {
    ...
    stage('Deploy to JabarCloud') {
      steps {
        sh 'ssh -o StrictHostKeyChecking=no $SERVER_USERNAME@$SERVER_HOST "cd digiteam/backend-v3 && \
        sed -i \\"s/image:.*/image: $IMAGE_DEPLOY-v3:$BUILD_NUMBER/g\\" digiteam-api-research-v3.yaml && \
        ./update-config.sh"'
      }
    }
