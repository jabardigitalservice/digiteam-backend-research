  ...
  stages {
    ...
    stage('Docker Push') {
      steps{
        ...
        sh 'docker push $IMAGE_NAME-v4:$BUILD_NUMBER'
      }
    }
    stage('Deploy to JabarCloud') {
      steps {
        sh 'ssh -o StrictHostKeyChecking=no $SERVER_USERNAME@$SERVER_HOST "cd digiteam/backend-v4 && \
        sed -i \\"s/image:.*/image: $IMAGE_DEPLOY-v4:$BUILD_NUMBER/g\\" digiteam-api-research-v4.yaml && \
        ./update-config.sh"'
      }
    }
